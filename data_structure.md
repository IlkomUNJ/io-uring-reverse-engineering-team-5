...
Structure name       | Defined in            | Attributes                                                                 | Caller Functions Source         | Source caller           | Usage
---------------------|------------------------|----------------------------------------------------------------------------|----------------------------------|--------------------------|-----------------------------
io_issue_def         | io_uring/opdef.h       | needs_file, pollin, pollout, hash_reg_file, ioprio, vectored, async_size, prep, issue | io_uring_get_opcode             | io_uring/opdef.c        | function parameter
                     |                        |                                                                            | io_uring_op_supported            | io_uring/opdef.c        | function parameter
                     |                        |                                                                            | io_uring_optable_init            | io_uring/opdef.c        | local variable
io_cold_def          | io_uring/opdef.h       | name, cleanup, fail                                                        | io_uring_get_opcode              | io_uring/opdef.c        | return value
                     |                        |                                                                            | io_uring_optable_init            | io_uring/opdef.c        | local variable
io_issue_defs[]      | io_uring/opdef.c       | Array of io_issue_def                                                      | io_uring_get_opcode              | io_uring/opdef.c        | array access
                     |                        |                                                                            | io_uring_op_supported            | io_uring/opdef.c        | array access
                     |                        |                                                                            | io_uring_optable_init            | io_uring/opdef.c        | initialization
io_cold_defs[]       | io_uring/opdef.c       | Array of io_cold_def                                                       | io_uring_get_opcode              | io_uring/opdef.c        | array access
                     |                        |                                                                            | io_uring_optable_init            | io_uring/opdef.c        | initialization
io_open              | io_uring/openclose.c   | file, dfd, file_slot, filename, how, nofile                                | io_openat_prep                   | io_uring/openclose.c    | function parameter
                     |                        |                                                                            | io_openat2_prep                  | io_uring/openclose.c    | function parameter
                     |                        |                                                                            | io_openat                        | io_uring/openclose.c    | local variable
                     |                        |                                                                            | io_open_cleanup                  | io_uring/openclose.c    | function parameter
io_close             | io_uring/openclose.c   | file, fd, file_slot                                                        | io_close_prep                    | io_uring/openclose.c    | function parameter
                     |                        |                                                                            | io_close                         | io_uring/openclose.c    | local variable
                     |                        |                                                                            | io_close_fixed                   | io_uring/openclose.c    | local variable
                     |                        |                                                                            | io_install_fixed_fd_prep         | io_uring/openclose.c    | local variable
                     |                        |                                                                            | io_install_fixed_fd              | io_uring/openclose.c    | local variable
io_fixed_install     | io_uring/openclose.c   | file, o_flags                                                               | io_install_fixed_fd_prep         | io_uring/openclose.c    | function parameter
                     |                        |                                                                            | io_install_fixed_fd              | io_uring/openclose.c    | local variable
io_kiocb             | io_uring/openclose.h   | req, sqe                                                                    | io_openat_prep                   | io_uring/openclose.c    | local variable
                     |                        |                                                                            | io_openat                        | io_uring/openclose.c    | function parameter
                     |                        |                                                                            | io_open_cleanup                  | io_uring/openclose.c    | function parameter
io_ring_ctx          | io_uring/openclose.h   | ctx, issue_flags, offset                                                   | __io_close_fixed                 | io_uring/openclose.c    | function parameter
                     |                        |                                                                            | io_openat                        | io_uring/openclose.c    | function parameter
                     |                        |                                                                            | io_open_cleanup                  | io_uring/openclose.c    | function parameter
io_poll_update       | io_uring/poll.c        | file, old_user_data, new_user_data, events, update_events, update_user_data | (none)                          | (none)                  | local variable
io_poll_table        | io_uring/poll.c        | pt, req, nr_entries, error, owning, result_mask                            | io_poll_queue_proc               | io_uring/poll.c         | function parameter
io_poll              | io_uring/poll.c        | head, events, wait                                                          | io_poll_remove_entries           | io_uring/poll.c         | local variable
                     |                        |                                                                            | __io_poll_execute                | io_uring/poll.c         | function parameter
                     |                        |                                                                            | io_pollfree_wake                 | io_uring/poll.c         | function parameter
io_poll_wake         | io_uring/poll.c        | wait, mode, sync, key                                                       | io_pollfree_wake                 | io_uring/poll.c         | function parameter
io_poll_mark_cancelled | io_uring/poll.c      | (none)                                                                      | io_poll_cancel_req               | io_uring/poll.c         | local variable
io_poll_get_ownership | io_uring/poll.c       | (none)                                                                      | io_poll_get_ownership_slowpath   | io_uring/poll.c         | return value
io_poll_req_insert   | io_uring/poll.c        | hash_node                                                                    | io_poll_add_hash                 | io_uring/poll.c         | function parameter
io_poll_execute      | io_uring/poll.c        | mask, res                                                                    | __io_poll_execute                | io_uring/poll.c         | function parameter
io_poll_remove_entry | io_uring/poll.c        | head                                                                          | io_poll_remove_entries           | io_uring/poll.c         | function parameter
io_poll_check_events | io_uring/poll.c        | tw, poll_refs, events, cqe.res                                               | io_poll_execute                  | io_uring/poll.c         | local variable
io_poll_add_hash     | io_uring/poll.c        | (none)                                                                        | (none)                           | io_uring/poll.c         | poll request insertion
